
#include "roman_numeral_operations_list.h"
#include "story_reader.h"

ROMAN_NUMERAL_TEST_OPERATION * rntList_create(int listSize){
    ROMAN_NUMERAL_TEST_OPERATION * romanNumeralTestList;
    romanNumeralTestList = calloc(listSize * sizeof(ROMAN_NUMERAL_TEST_OPERATION), 1);
    return romanNumeralTestList;
}



bool rntList_parse(ROMAN_NUMERAL_TEST_OPERATION * romanNumeralTestList, char * fileName){
    TEST_STORY * story;
    int listPosition = 0, lineLength;
    int i, j; //iterators
    char currentChar;
    STORY_PARSE_STATE parseState;

    story = story_create(fileName);
    if(story == NULL){
        printf("Unable to create story, error number %d\r\n", errno);
        return 0;
    }

    if(!story_open(story)){
        printf("Unable to open story, error number %d\r\n", errno);
        return 0;
    }

    while(story_read_line(story)){
        lineLength = strlen(story->readBuffer);
        parseState = OPERAND_1;

        for(i = 0, j = 0; i < lineLength; i++){
            currentChar = story->readBuffer[i];

            switch(parseState){

                case OPERAND_1:
                    if(currentChar == '+' || currentChar == '-'){
                        romanNumeralTestList[listPosition].operator = currentChar;
                        j = 0;
                        parseState = OPERAND_2;

                    }else{
                        romanNumeralTestList[listPosition].operand1[j++] = currentChar;
                    }               
                    break;
                case OPERAND_2:
                    if(currentChar == '='){
                        j = 0;
                        parseState = RESULT;
                    }else{
                        romanNumeralTestList[listPosition].operand2[j++] = currentChar;
                    }               
                    break;
                case RESULT:
                    if(currentChar == '\r' || currentChar == '\n'){
                        parseState = PARSE_COMPLETE;
                    }else{
                        romanNumeralTestList[listPosition].result[j++] = currentChar;
                    }                   
                    break;
            }

        }

        listPosition++;
    }


    if(!story_close(story)){
        printf("Unable to close story, error number %d\r\n", errno);
        return 0;
    }

    return 1;
}

bool rntList_destroy(ROMAN_NUMERAL_TEST_OPERATION * romanNumeralTestList){
    free(romanNumeralTestList);
    romanNumeralTestList = NULL;
    return 1;
}

bool rntList_operation_valid(ROMAN_NUMERAL_TEST_OPERATION * romanNumeralTestList){
    if( strlen(romanNumeralTestList->operand1) > 0 ){
        printf("Checking roman numeral operation: %s %c %s = %s \r\n", romanNumeralTestList->operand1, romanNumeralTestList->operator, romanNumeralTestList->operand2, romanNumeralTestList->result); 
    }
    return 1; 
}
